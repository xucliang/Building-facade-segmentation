...
def compute_votes(edgelets, model, threshold_inlier=1):
...
def remove_inliers(model, edgelets, threshold_inlier=10):
    ...
    """
    inliers = compute_votes(edgelets, model, 5) > 0
    locations, directions, strengths = edgelets
    locations = locations[~inliers]
    directions = directions[~inliers]
    strengths = strengths[~inliers]
...
def compute_homography_and_warp(image, vp1, vp2, clip=True, clip_factor=4):
...
#        if coefficients[0]*480+coefficients[1]
...
def rectify_image(image, clip_factor, algorithm='independent',
                  reestimate=False):
    ...
    if algorithm == 'independent':
        ...
        vp1 = ransac_vanishing_point(edgelets1, 8000, threshold_inlier=2)

        # Remove inlier to remove dominating direction.
        edgelets2 = remove_inliers(vp1, edgelets1, 2)
    ...
    # Find second vanishing point
       ...
       # edgelets6 = remove_inliers(vp5, edgelets5, 10)
        ...
        # Find second vanishing point
       # vp6 = ransac_vanishing_point(edgelets6, 10000, threshold_inlier=2)

    elif algorithm == '3-line':
        focal_length = None
        vp1, vp2 = ransac_3_line(edgelets1, focal_length,
                                 num_ransac_iter=3000, threshold_inlier=1)
    else:
        raise KeyError(
...
import pandas as pd
 ...
 for i in range(len(df1[0])-1):
     if (df1[0][i+1]-df1[0][i])>=9:
      df2[0:][i]=df1.iloc[i,:]
      df2[0:][i+1]=df1.iloc[i+1,:]

df2[0:][0]=df1.iloc[0,:]
df2[0:][-1]=df1.iloc[-1,:]
df3=df2[~(df2==0).all(1)]#行是1列是0

for i in range(len(df3) - 1):
    if (df3[i + 1][0] - df3[i][0]) <= 9:
        df3[i+1] = np.zeros((1, 3))
df3[-1]=df2[0:][-1]
